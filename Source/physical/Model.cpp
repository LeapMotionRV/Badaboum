...
namespace physical 
{
	Model::Model(unsigned int countParticles)
	{
		...
		m_pGraphHookForce = new GraphHookForce(0.1f, 2.f);
		m_pGraphHookForce->setGraph(m_pLine->getGraph());

		m_pGraphBrakeForce = new GraphBrakeForce(0.01f);
		m_pGraphBrakeForce->setGraph(m_pLine->getGraph());
	}
